...
int test_cluster(test_cluster_cfg_t *test_cfg) {
    ...
    do {
        ...
        for (int id = 0; id < test_cfg->clusters; id++) {
            test_cluster_args_t *arg = test_cfg->args[id];
            offload_snitchCluster(test_cfg->function_test, test_cfg->function_trampoline,
                                  (void *)arg, stack_cluster_ptr[id], test_cfg->clusterIds[id]);
        }
        // Enable interrupts
        set_mie(1);

        // Handle shared_data.device_to_host/shared_data.host_to_device communication with timeout
        int timed_out = 0;
        int done = 0;
        while (done != test_cfg->clusters && !timed_out) {
            ...
            if (shared_data.device_to_host != 0) {
                ...
                shared_data.device_to_host = syscall_addr;
            }
